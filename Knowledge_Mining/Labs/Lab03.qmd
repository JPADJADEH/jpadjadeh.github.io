---
title: "EPPS 6323: Lab 3 - Machine Learning with Survey Data (TEDS 2016)"
author: "JP ADJADEH"
format: html
editor: visual
---
...
Loading all the packages we need and pulling in the TEDS 2016 dataset.
...
Creating cleaner factor versions of the key variables and dropping NAs so the models run smoothly.
...
Scaling the numeric variables so everything is on the same scale before running k-means.
...
Using the elbow and silhouette methods to figure out how many clusters make sense.
...
Running k-means with 3 clusters based on the results above.
...
Looking at what makes each cluster different.
...
Splitting 70/30 so we can train on most of the data and test on the rest.
...
Putting all three models side by side to see which one performed best on the test set.
